...
function layer(type, name, props = {}, tracks = {}, children = null) {
  ...
  if (type === "confetti") Object.assign(base.props, { burst: 500, count: 70, power: 1, seed: 7 });
...
function buildLogoReveal() {
  ...
  return project([ring, mark, brand, tag]);
}
...
function buildQuoteCard() {
  ...
  return project([mark, bar, quote, author]);
}
...
function buildLowerThird() {
  ...
  return project([bgBar, strip, name, title]);
}
...
function buildSubscribe() {
  ...
  return project([button, cta, bell, sub]);
}
...
function buildPromoFlash() {
  ...
  return project([morphA, morphB, headline, chip, confA, confB]);
}
...
export const TEMPLATES = [
  { id: "logo-reveal", name: "Logo Reveal", description: "Mark pops in with an overshoot ease while the brand name rises letter by letter.", accent: AMBER, buildProject: buildLogoReveal },
  { id: "quote-card", name: "Quote Card", description: "Serif pull-quote with per-character rise and a subtle amber rule.", accent: VIOLET, buildProject: buildQuoteCard },
  { id: "lower-third", name: "Lower Third", description: "Broadcast-style name bar that slides in and locks into place.", accent: AMBER, buildProject: buildLowerThird },
  { id: "countdown", name: "Countdown 5-4-3-2-1", description: "JetBrains Mono counter with a depleting progress ring.", accent: BLUE, buildProject: buildCountdown },
  { id: "subscribe-cta", name: "Subscribe CTA", description: "Pulsing button, ringing bell and a follow-up line — end-card ready.", accent: CORAL, buildProject: buildSubscribe },
  { id: "promo-flash", name: "Promo Flash", description: "Scrambled headline, morphing accents and two confetti bursts.", accent: MINT, buildProject: buildPromoFlash },
];
