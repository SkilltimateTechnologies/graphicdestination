...
function shapePtsOf(shapeId, cornerR) {
...
function colorAt(obj, prop, time) {
...
function valueAt(obj, prop, time) {
  const track = obj.tracks[prop];
  if (!track || track.length === 0) return obj.props[prop];
  if (time <= track[0].t) return track[0].v;
  const last = track[track.length - 1];
...
const fxDuration = (fx, n, dur = 5000) => (fx.type === "wave" ? dur : (n * FX_STAG + FX_CDUR + 200) / (fx.speed || 1));

function charFx(fx, i, n, time, ch) {
...
function confettiParticles(obj) {
...
function clipTransition(P, time) {
  const td = P.tDur || 500;
...
function highlightFlick(u, seed) {
  if (u <= 0) return 0;
  if (u >= 0.8) return 1;
...
function worldCameraAt(P, time, fallbackCenter) {
...
function StageObject({ obj, time, stage, selected, onDown, onEnterClip, displayValue, onResize, onRotate, interactive }) {
  const P = obj.props;
