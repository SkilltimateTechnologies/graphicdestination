...
/* Maps panel — three categories, live previews, electric pop style:
   ...
   R10: back on the LEFT rail as a standard drawer (left:84 · width 268 · the
   shared .gd-panel shell/colors/zIndex) — it briefly lived as a custom
   right-side overlay. The cyan trace identity stays inside the content. */
...
/* shared left-rail drawer shell (R10): identical geometry/colors/zIndex to
   every other .gd-panel; only the cyan trace accent remains map-specific */
const S = {
  wrap: { position: "absolute", left: 84, top: 12, width: 268, maxHeight: "calc(100% - 24px)", background: C.bg2, border: `1px solid ${C.line}`, borderRadius: 8, zIndex: 30, boxShadow: "0 12px 40px rgba(0,0,0,.5)", display: "flex", flexDirection: "column", overflow: "hidden" },
  head: { display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderBottom: `1px solid ${C.line}`, flexShrink: 0 },
  title: { fontSize: 13, fontWeight: 700, color: C.txt, flex: 1 },
  close: { background: "none", border: "none", color: C.dim, cursor: "pointer", fontSize: 16, padding: 4 },
  tabs: { display: "flex", gap: 6, padding: "10px 12px 8px", flexShrink: 0 },
  tab: (on) => ({ flex: 1, padding: "6px 0", fontSize: 11.5, fontWeight: 700, borderRadius: 7, border: "1px solid " + (on ? "#38BDF8" : C.lineStrong), background: on ? "rgba(56,189,248,0.14)" : "transparent", color: on ? "#7DD3FC" : C.dim, cursor: "pointer" }),
  search: { margin: "0 12px 8px", padding: "7px 10px", fontSize: 12, background: C.bg0, border: `1px solid ${C.lineStrong}`, borderRadius: 7, color: C.txt, outline: "none", flexShrink: 0 },
  body: { flex: 1, minHeight: 0, overflowY: "auto", padding: "2px 12px 12px" },
  sec: { fontSize: 10.5, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", color: C.faint, margin: "12px 0 6px" },
  grid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 },
  cell: { background: C.bg0, border: `1px solid ${C.line}`, borderRadius: 9, padding: 6, cursor: "pointer", textAlign: "center" },
  label: { fontSize: 10.5, color: C.dim, marginTop: 4, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  chipRow: { display: "flex", alignItems: "center", gap: 6, padding: "4px 0", borderBottom: `1px solid ${C.line}` },
  ...
  num: { width: 52, padding: "3px 5px", fontSize: 10.5, background: C.bg0, border: `1px solid ${C.lineStrong}`, borderRadius: 5, color: C.txt, outline: "none" },
  insert: { margin: "10px 12px 12px", padding: "9px 0", fontSize: 12.5, fontWeight: 800, borderRadius: 8, border: "none", background: "linear-gradient(135deg,#38BDF8,#818CF8)", color: "#fff", cursor: "pointer", flexShrink: 0 },
  hint: { fontSize: 10.5, color: C.faint, padding: "0 12px 10px", flexShrink: 0 },
};
...
function CountryThumb({ cc }) {
  ...
  const w = 104, h = Math.max(46, Math.min(110, (w * ch) / cw));
  return (
...
export default function MapsPanel({ addObject, setMapsOpen }) {
  ...
  return (
    <div className="gd-panel" data-maps-panel style={S.wrap}>
      <div style={S.head}>
      ...
      {tab === "world" && (
        <>
          <div style={S.body}>
            ...
            <div style={{ background: C.bg0, border: `1px solid ${C.line}`, borderRadius: 9, padding: 6 }}>
            ...
            {hi.map((x, i) => {
              ...
              return (
                <div key={x.cc} style={S.chipRow}>
                  ...
                  <div style={{ flex: 1, fontSize: 11, color: C.txt, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{nm}</div>
                  <input style={S.num} type="number" min={0} step={100} value={x.t} title="appears (ms)" onChange={(e) => setHiField(i, "t", e.target.value)} />
                  <span style={{ fontSize: 9.5, color: C.faint }}>→</span>
            ...
            {composerList.map(({ cc, n }) => (
              <div key={cc} style={{ ...S.chipRow, cursor: "pointer" }} onClick={() => addHi(cc)}>
                ...
                <div style={{ flex: 1, fontSize: 11, color: C.dim }}>{n}</div>
