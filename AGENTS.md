...
| Emoji (Fluent 3D) | 169 Microsoft Fluent Emoji (MIT, "Smileys & Emotion") as 3D PNGs in `client/public/emoji/fluent/3d/`; `engine/emoji.js` `buildEmojiClip` wraps ONE image layer in a seamlessly-looping clip with the in→hold→out motion grammar (heartbeat/spin/rock/bob/pulse by keyword); inserts normalized to a 100×100 movable grouped clip. Rail `EmojiPanel`: featured teaser (click inserts) + → arrow swaps the panel INLINE to the full library (search + cats + animated/still) — NO modal. Assets fetched by `scripts/fetch-fluent-emoji.mjs` | `check-emoji` (30) |
