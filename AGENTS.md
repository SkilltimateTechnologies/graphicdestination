...
| Emoji (Fluent 3D) — RETIRED from picker | 169 Microsoft Fluent Emoji PNGs + `engine/emoji.js` stay engine-side for BACK-COMPAT only (old image/clip layers keep rendering; assets fetched by `scripts/fetch-fluent-emoji.mjs`). The picker is the **SVG Icons library**: admin-managed store (`svg_icons` table, `server/svgSanitize.js` allowlist — script/on*/foreignObject/external refs stripped BEFORE store; admin-only POST/PUT/DELETE `/api/svg-icons`), `IconsPanel.jsx` (search + cats), insert = plain `image` with an inline-SVG data-URI src at `DEFAULT_INSERT_SIZE` (`engine/svgIcon.js` — never blob:, canvas-taint-safe) | `check-emoji` (30), `check-svg-icons` (16), `server/test-svg-sanitize` (25), `server/test-svg-icons` (19) |
...
| Timeline | ONE LAYER PER ROW (`const rows = ctxLayers.map(o => [o])`) — After-Effects/CapCut model: no packing, a lane never shows two labels, moving a clip in time never reshuffles. Double-click a lane name to rename inline (`renameLayer`). `packRows` stays in model.js (start-sorted default + `{stable}`) for guards/other callers but the timeline no longer uses it; the bar-drag row-jump/gap-pill machinery is vestigial. Plus: pinned scrub ruler, lock/hide toggles, scrub-follow playhead chase, per-prop kf glyphs | `check-timeline` (49), `check-r8w1`, `check-r9w1`, `check-editor-w1` (browser) |
| Canvas selection + groups | plain-drag on empty canvas = MARQUEE rubber-band select (`objectsInRect` in model.js; locked/hidden excluded). Camera pan = middle-mouse / Alt+drag. Multi-select = Ctrl/Cmd OR **Shift**-click, on canvas objects AND timeline lanes. Groups are FOLDERS (the `clip` type) nested **≤3 levels** (folder 1›2›3 — `groupSelection` blocks at `path.length ≥ 3`); group/ungroup via ⌘G / Inspector / timeline glyph pair / right-click, depth crumb in the timeline. (A CapCut multi-clip-track model was built then REVERTED — folders won; see docs/UPDATES.md.) | `check-marquee` (11), `check-editor-w1` (browser) |
| Templates | insert as movable content-sized groups; camera tracks only via buildProject (root-level). EDITABLE: `templates` store (scope `global` admin / `user` personal) overlays built-ins — a global row whose slug matches a built-in id OVERRIDES it (`mergeTemplates` in templates.js; code never mutated), panel badges Global/Mine, Save-as-template writes projectJson via `POST /api/templates` (validated server-side by templateValidate) | `check-templates`, `server/test-templates` (23), `check-r8w4` (browser override + save flow) |
| Settings | `/settings` brand kits + 4-tier text styles + default bg (black fallback); `user_settings` table; editor brand switcher applies via `kitToBrand`. ACTIVE brand colors are the insert DEFAULTS (`brandDefaultFor` in editor/model.js: shape fill/stroke, chart + confetti `colors[]` palettes, counter ringC, map strokes, kit accent; explicit props always win, no brand → fixed swatches) | `check-r9w3` (71), `server/test-settings` (38), `check-editor-w1` (browser brand inserts) |
| Panels | mutually exclusive rail panels (openOnly); thumbs = representative hold frame (never t=0), hover plays | `check-r8w4` (69) |
| Uploads hub | ONE rail panel (`UploadsPanel.jsx`) for images + audio (Image/Audio panels retired); dropzone → `onUploadFiles` MIME-routed; project-scoped by default (`assets.project_id`, `?project=` filter) with cross-project "All projects" search; images insert by reference, audio attaches as track. Server: magic-byte sniffing (`assetSniff.js` — declared mime is a hint, bytes must verify; stored mime = sniffed), per-kind caps (3/5 MB), `X-Content-Type-Options: nosniff` on serving | `server/test-assets-hub` (15), `server/test-assets`, `server/test-audio`, `check-r10` (A1/B10), `check-r8w4` (8e) |
...
- Emoji assets: `client/public/emoji/fluent/3d/*.png` (Fluent Emoji, MIT — see that dir's LICENSE.txt); regenerate with `node scripts/fetch-fluent-emoji.mjs`. Panels: `IconsPanel.jsx` (admin SVG library — the current picker). Removed over time: legacy bezier `IconsPanel.jsx`, `EmojiLibrary.jsx`, `EmojiPanel.jsx` (Fluent picker, B phase 4).
