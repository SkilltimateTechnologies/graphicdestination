...
| Timeline | row-jump deadzone (60%/20px), gap pills + ripple-close, lock/hide icon toggles, scrub-follow playhead chase, per-prop kf glyphs. Scrub ruler is PINNED (sticky band above the horizontal scroller, `scrollLeft`-synced via `rulerRef.style.transform`) so it stays visible when many lanes scroll — `rulerRef` still points at the tick strip so cursor→time math is unchanged | `check-timeline` (45), `check-r8w1`, `check-r9w1` |
